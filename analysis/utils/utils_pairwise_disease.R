...
# 4. part : perform partial correlation 
...
get.continguency.columns= function(A, pids, phecodes, data, disease_ontology){
  ...
  A.tidy2= A.tidy %>% 
    ...
    select(disease1,disease2,a,b,c,d)
...
get.pairwise.metric = function(A.tidy){
  ...
  A.res= A.tidy %>% 
    ...
    mutate(fisher.p = fisher.test(matrix(c(a,b,c,d), nrow= 2, byrow = T))$p.value,
            ...
            fisher.p.adj= p.adjust(fisher.p, method = "fdr"),
...
get.part.corr = function(pids, phecodes, data){
  ...
  #calculate part. correlation, based on a ML estimation
  ...
  get_confint= function(t,n = length(pids), z= 1.96){
  ...
  t_df= t1_df %>% 
    ...
    left_join(get_confint(as.data.frame(t2[,]))) %>% ## add confident intervalls (for pcor by psych pacakge(t3))
    #rowwise() %>% 
