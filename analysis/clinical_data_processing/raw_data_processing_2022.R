...
turn_to_vec = function(column){
  x= lapply(lapply(column, function(x) paste0(x, collapse=" ,")), FUN = noquote) %>%
    as.character(.)
...
demo$demografie.gewicht

#### weight, height and BMI
x = turn_to_vec(column= demo$demografie.gewicht)
x= str_replace_all(x, " ,", ";")
x= str_replace_all(x, ",", ".")
demo$demografie.gewicht = x
weight =demo %>% select(patient_id, entry_date, demografie.gewicht)%>%slice(1:1000)%>%
...
ef.echo$diagnostik.echo.lv_ef = turn_to_vec(column = ef.echo$diagnostik.echo.lv_ef)
...
hist(ef.echo2$diagnostik.echo.lv_ef)
...
saveRDS(ef.full, file = file.path(directory, "EF_df.rds"))
...
echo_df= lv_edd.echo.max %>% rename(edd.max= maxLv.EDD)  %>%
  ...
  full_join(ef.full  %>% group_by(patient_id) %>%
...
filter(weight<500 & weight > 20)%>% as_tibble()
