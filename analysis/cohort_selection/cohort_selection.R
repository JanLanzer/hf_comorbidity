...
pids.I42 = icd10 %>% 
  ...
  group_by(pid) %>%
  #filter(any(icd3 == "I42")) %>% 
  filter(any(icd4 %in% c("I42.1", "I42.2", "I42.3", "I42.4", "I42.6", "I42.7"))) %>% 
  arrange(pid)%>%  
  distinct(pid) %>%
  pull(pid)
...
ggVennDiagram(list(nyha= as.integer(pids.nyha), icd10 =pids.icd10))
...
saveRDS(pids.list,
        file = "T:/fsa04/MED2-HF-Comorbidities/lanzerjd/data_output/pidslist_v2021.rds" )
